...
Autogenerated from [std/net/http/server.luau](/.seal/typedefs/std/net/http/server.luau). *seal* is best experienced with inline, in-editor documentation. Please see the linked typedefs file if this documentation is confusing, too verbose, or inaccurate.
