...
`prompt.--     in reverse order.`

`prompt.--     Overreliance on this function might make users hate your CLI! Only use it when necessary please.`

`prompt.--     To prevent excessive annoyance, this confirmation may be cheesed by re-pasting the original OTP, causing a message`

`prompt.--     with the reversed OTP to show up and be copy/pasted. For truly dangerous operations, this cheese may be disabled.`
...
`prompt.--         packages_dir:remove()`

`prompt.--         -- displays`

`prompt.--         --[[`

`prompt.--         Delete all installed packages?`

`prompt.--         To confirm, retype your OTP in reverse order.`

`prompt.--         For example, OTP: a-b-c means you type c-b-a and press enter to confirm.`

`prompt.--         ⚠️ This deletes all your changes.`
...
`prompt.--         -- to confirm, user types "octopi-strinky-stinky" and presses enter`
