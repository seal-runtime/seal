...
/// Parses the `bytes` argument (a plain number or a FileSize) into a max byte count.
fn parse_bytes(value: LuaValue, function_name: &'static str) -> LuaResult<Option<u64>> {
    match value {
        ...
        LuaValue::UserData(ud) => {
            ...
            wrap_err!("{}: expected bytes to be a number or FileSize (from @std/fs/filesize), got a different kind of userdata: {}", function_name, type_name)
        },
        other => {
            wrap_err!("{}: expected bytes to be a number, FileSize, or nil, got: {:?}", function_name, other)
        }
    }
}

/// Parses the `timeout` argument (a Duration userdata) into a std Duration.
fn parse_timeout(value: LuaValue, function_name: &'static str) -> LuaResult<Option<Duration>> {
    match value {
        ...
        LuaValue::UserData(ud) => {
            let type_name = ud.type_name()?.unwrap_or("userdata (missing __type metafield)".to_string());
            wrap_err!("{}: expected timeout to be a Duration (from @std/time), got a different kind of userdata: {}", function_name, type_name)
        },
        LuaValue::Number(_) | LuaValue::Integer(_) => {
            wrap_err!("{}: timeout should be a Duration (from @std/time), not a plain number", function_name)
        },
        other => {
            wrap_err!("{}: expected timeout to be a Duration or nil, got: {:?}", function_name, other)
        }
    }
}
...
pub(crate) fn read_from_stdin(max_bytes: Option<u64>, timeout: Option<Duration>) -> io::Result<(Vec<u8>, bool)> {
    match timeout {
...
fn input_read(luau: &Lua, mut multivalue: LuaMultiValue) -> LuaResult<(LuaValue, bool)> {
    let function_name = "input.read(bytes: (number | FileSize)?, timeout: Duration?)";

    let max_bytes = parse_bytes(multivalue.pop_front().unwrap_or(LuaNil), function_name)?;
    let timeout = parse_timeout(multivalue.pop_front().unwrap_or(LuaNil), function_name)?;

    let (buffy, more_to_read) = match read_from_stdin(max_bytes, timeout) {
        Ok(result) => result,
        Err(err) => {
